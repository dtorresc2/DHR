...
public class QuerysBitacora {
    ...
    public void consultaAvanzada(final JSONObject jsonBody, QuerysBitacora.VolleyOnEventListener callback) {
        mCallBack = callback;

        StringRequest stringRequest = new StringRequest(Request.Method.POST, mContext.getResources().getString(R.string.API) + "bitacora/consulta/avanzada",
                response -> mCallBack.onSuccess(response),
                error -> mCallBack.onFailure(error)) {
